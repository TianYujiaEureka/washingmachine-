...
#include "USB_To_Usart.h"	
	void GetSHTDataToGlobParams(int num)
{
	int sendDataLen=0;
	int sendDataLen1=0;
...
{ int num=0;
  ...
  u8 key;           //保存键值
	...
	initDataFrame(0xDC201001661CBFC2,0x123456);//初始化数据协议，并设置协议中用到的设备id和登录密码
 ...
 /*
	*/
	...
	{	if(WIFI_USART_RX_STA>0x8000){
				...
				delay_ms(11);
				OLED_ShowString(0,50, (unsigned char *)" >80000  ",12);
				if(ParseServerData(WIFI_USART_RX_BUF, WIFI_USART_RX_STA^0x8000)){
				OLED_ShowString(0,50, (unsigned char *)"……&……80000 ",12);
					if(dataFrame.mOrderConnect[0]==1){
						LED0=1;
						OLED_ShowString(0,50, (unsigned char *)"    %1%      ",12);
					}else{
						LED0=0;
						OLED_ShowString(0,50, (unsigned char *)"     %0%   ",12);
					}
		...
		{						
			...
			{				 
				...
				case KEY2_PRES:	//
					LED0=1;
					OLED_ShowString(0,50, (unsigned char *)"SEND DATA",12);
